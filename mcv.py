...
def build_manifest(
    corpus_dir: Path, split: str, clips_out_dir: Path, manifest_path: Path, workers: int
) -> int:
    if split == "train":
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool, open(
        ...
        for (dst, text), duration in zip(
            rows, tqdm(pool.map(convert_clip, jobs), total=len(jobs), desc=split)
        ):
            if not text:
                continue
            out.write(
...
def prepare_mcv_dataset(args):
    ...
    if pending_splits:
        ...
        if corpus_dir:
            print(
                f"Found existing extracted corpus at {corpus_dir}, skipping download/extraction"
            )
    ...
    for split in args.splits:
        ...
        if split not in pending_splits:
            ...
            print(
                f"{split}: manifest already exists -> {manifest_path} ({counts[split]} utterances), skipping"
            )
            continue
        counts[split] = build_manifest(
            corpus_dir, split, output_dir / "wavs", manifest_path, args.workers
        )
