...
`prepare_data.py` fetches the sources listed under `data.sources` in `config.yaml` and
writes NeMo manifests into `data/`:

```bash
python prepare_data.py                      # every configured source
python prepare_data.py --dataset fleurs     # one source, for a fast first pass
```

The full Bengali corpus is large — around 85G downloaded, ~104G of wavs on disk — so
start with `--dataset fleurs` to check the pipeline end to end before committing to
that.

Manifest paths are written relative to the project root, so run every command from
this directory.

The run behind the published adapter used:
...
If you already have the audio locally, point `data/wavs` at it (a symlink works) and
skip the download.

## Run

Once `data/` holds manifests:
...
data:
  sources:
    - type: fleurs
      name: fleurs
    - type: cv_parquet          # local Common Voice parquet shards
      name: cv
